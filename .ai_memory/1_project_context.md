...
- 默认模式和自定义模式的软件 precheck 都会在终端同一行刷新已完成数量，并在结束时刷新为完成行；Skill、MCP、CLI 状态扫描也同一行刷新完成数量，分别以 `[检查] Skill`、`[检查] MCP`、`[检查] CLI` 为进度标签，结束时只保留完成行。Skill 单项入口复用 `Get-SkillBundleComponentStatus -IncludeSkills`；MCP 使用 TUI 专用轻量延迟保证过程可见，但最后 `100%` 后不再额外等待。
