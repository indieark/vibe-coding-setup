...
- 自定义模式聚焦任务动作：检查并安装/更新软件、检查并安装/更新套件、检查并安装/更新 Skill、检查并安装/更新 MCP、检查并安装/更新 CLI；工作台先显示 `[可执行动作]`，只有已有可执行选择后才把 `开始执行` 显示为第一项，并在动作区下方显示 `[当前选择]`；选择 `开始执行` 后直接返回执行参数，不再进入命令确认页。
...
- winget 安装如果已经输出成功但进程迟迟不退出，脚本会短暂等待后结束卡住的 winget 外壳并继续后续检测；如果已经输出成功但最终退出码缺失或异常，也按成功处理；如果没有成功输出但安装后复查确认应用已安装，同样按成功处理并在摘要显示“安装后复查”；复查仍未通过时才按原始退出码处理失败。
...
- 非 Microsoft Store 的 winget 应用应在 `manifest/apps.json` 明确配置 `wingetSource: "winget"`，避免默认搜索 `msstore` 时因 Store 源超时误伤安装 / 更新；只有 Store 包如 Codex Desktop 使用 `wingetSource: "msstore"`。
- 应用主来源失败后，`Install-AppFromDefinition` 先做 post-check，再读取统一 `$fallback` 对象尝试 winget / release / URI fallback；所有 fallback 失败原因会汇总进最终错误，避免只提示“无可用 fallback”而丢失根因。
