...
15. 创建 Codex 默认工作目录。
16. 按 `order` 串行消费预检查结果；安装项显示“准备安装”，更新项显示“准备更新”，检查失败项直接进入失败摘要；实际安装阶段复用该预检查决策，不再逐项重复播报“预检查 ...”。
