...
Profile 交互菜单会把 `0`、`00` 和每个套件展示为两行：第一行是名称和暗灰色括号说明，说明过长时按当前终端宽度截断为 `...`；第二行是带不同颜色的 Skill / MCP / CLI 数量摘要。`0` 会显示全部离线 Skill 数、MCP 0、CLI 0，`00` 会显示 Profile 并集 Skill 数、MCP 数和 CLI 数。TUI 复选页光标停在某个套件时，会临时展示该套件将写入的 MCP 和将处理的 CLI 前置依赖；默认交互菜单在用户输入后、执行前也会输出同样摘要。可输入序号/名称，多个可用英文逗号、中文逗号或顿号分隔；输入 `0` 安装全部 Skill，输入 `00` 安装所有套件。
...
Profile 可以同时引用 bundled skill、external skill、MCP 和前置依赖：

- bundled skill：`skills.zip` 内已有 `SKILL.md` 的离线目录，直接走三态导入。
