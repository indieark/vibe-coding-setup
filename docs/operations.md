...
默认安装和自定义模式的软件入口都会在应用预检查期间同一行刷新已完成数量，并在结束时刷新为完成行；Skill、MCP、CLI 状态扫描也遵循同一规则，并分别显示 `[检查] Skill`、`[检查] MCP`、`[检查] CLI` 进度标签。默认安装的插件安装段在交互式套件输入区前也会执行这组节能版组件状态扫描，并在 `全部 Skill`、`所有套件` 和各 Profile 行上标记已安装、部分安装、需更新、更新未知或未安装。winget 安装若已经报告安装完成但进程未退出，脚本会自动收尾后继续，不再长期停在“仍在运行”。

默认安装的套件状态标记与自定义模式中的“检查并安装/更新”都优先保证快速、离线友好和少副作用。Skill 更新状态只比较当前 `skills.zip` 的 meta 与本机已安装 meta；MCP 更新状态只比较 registry 期望配置与本机 Codex、Claude Desktop、Claude Code、Cursor、Gemini CLI、Antigravity 配置；CLI 入口只运行 `prereqs.yaml` 中的本地 `check` 命令，不检查远程最新版，所以已检测到的 CLI 仍可能显示“更新未知”。如果需要真实联网更新维护，需先为 registry 增加明确版本字段和检查命令，并提供超时、缓存和禁用策略。
...
默认安装模式进入 UAC 后不会再次显示 TUI，也不会把应用清单拆成多个位置参数；自定义模式和安全演练才会生成显式命令预览。如果你显式用 `.\bootstrap.cmd -Tui -DryRun` 这类命令进入 TUI，选择默认安装后仍会保留这些命令参数；进入自定义模式后，`-DryRun`、`-SkipCcSwitch` 等显式参数也会进入最终执行确认页。若本地缓存的 `common.psm1` 太旧，bootstrap 会刷新自举依赖；旧模块兼容兜底路径会静默旧风格组件进度，避免套件页重复显示 MCP 或 CLI。
