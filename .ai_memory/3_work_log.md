...
- 补充项目说明中的节能版“检查更新”语义：Skill 只基于当前 `skills.zip` 与本机 `.skill-meta.json` 做来源和 `source_revision` 对比；MCP 只检查本机是否已配置对应 server，不把用户自有配置与 registry 做更新或同步判定；CLI 只执行本地 `check` 命令判断是否存在，并将更新状态标为未知。
- 明确当前实现不是远程实时最新版查询，不会在 TUI 状态页无条件调用 `npm outdated`、`winget upgrade` 或 GitHub Release 查询。
- 记录未来真实维护型更新检查方向：先扩展 registry schema，再实现可缓存、可禁用、可超时的联网检查。
- 默认模式的插件安装套件输入区接入同一组节能版组件状态扫描：进入交互式 Profile 菜单前显示 `[检查] Skill`、`[检查] MCP`、`[检查] CLI` 进度，并在 `全部 Skill`、`所有套件` 和各 Profile 行展示已安装、部分安装、部分安装且需更新、需更新、更新未知或未安装；其中 MCP 只参与已配置 / 未配置判断，不产生需更新状态。
- 修正前置自举依赖标题和进度：进入 TUI 首屏前仍显示无编号 `获取依赖`；命令模式、默认安装、TUI 首屏选择默认安装后或 TUI 默认安装 UAC 续跑后显示 `步骤一：获取依赖`；本地缓存复用也输出 `同步 ... 100% 2/2 个依赖已完成`。
...
- 去掉默认安装里重复的 `[当前/总数]` 阶段提示：工作区准备、配置导入、插件安装只保留区域标题，最终完成提示也改为分区标题样式；主流程大区域之间保留两行空白，区域内小分块保持一行空白；套件输入区提示改为三行，套件选项压缩为两行，括号说明用暗灰色并按终端宽度截断，Skill / MCP / CLI 数量分色显示，所有套件也只显示 Skill / MCP / CLI 数量。
- TUI 运行模式首页在“现代化控制台安装向导”下补充三行说明：核心功能、具体能力和仓库地址；该说明只显示在首屏，不进入工作台和后续复选页。
...
- 拆分自定义模式组件状态扫描：Skill / MCP / CLI 单项入口只加载对应类型状态，套件入口才全量加载并显示 Skill / MCP / CLI 总览。
- 保持 Skill / MCP / CLI 单项选择跨类型累积，避免选择某一类时清空其它类型已选项。
- 修复新增裸中文文案导致 Windows PowerShell 5.1 `cmd -> powershell.exe -File` 启动解析崩溃的问题，新增中文仍使用 UTF-8 Base64 解码。
...
- 前置自举依赖开屏标题当时改为 `获取依赖`，不再显示默认模式专用的 `步骤一：获取依赖`；该历史规则已被 2026-05-04 新规则取代：只有进入 TUI 首屏前显示无编号 `获取依赖`，默认 / 命令主流程显示 `步骤一：获取依赖`。
