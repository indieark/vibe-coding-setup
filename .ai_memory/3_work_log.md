...
- 补充项目说明中的节能版“检查更新”语义：Skill 只基于当前 `skills.zip` 与本机 `.skill-meta.json` 做来源和 `source_revision` 对比；MCP 只检查本机配置是否与 registry 期望配置一致；CLI 只执行本地 `check` 命令判断是否存在，并将更新状态标为未知。
...
- 默认模式的插件安装套件输入区接入同一组节能版组件状态扫描：进入交互式 Profile 菜单前显示 `[检查] Skill`、`[检查] MCP`、`[检查] CLI` 进度，并在 `全部 Skill`、`所有套件` 和各 Profile 行展示已安装、部分安装、部分安装且需更新、需更新、更新未知或未安装。
