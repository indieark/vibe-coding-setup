...
18. 如果没有 `-SkipSkills`，再执行插件安装：获取 `skills.zip`，导入 Skill / 套件 / MCP / CLI 前置依赖；交互式默认安装在套件输入区前会先执行节能版 Skill / MCP / CLI 状态扫描，并把 `全部 Skill`、`所有套件` 和各 Profile 标记为已安装、部分安装、需更新、更新未知或未安装。
