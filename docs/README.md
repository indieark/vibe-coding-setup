...
2. 如果只想预览 Skill 导入风险，直接跑安全演练命令。
3. 如果看到 Skill 被跳过或备份，再看 [Skill 导入契约](skill-import.md)。
...
| 文档 | 适合什么时候读 | 不维护什么 |
| --- | --- | --- |
| [运行命令](operations.md) | 要本地运行、远程自举、dry-run 或按 Profile 导入 Skill | 不解释内部状态机 |
| [安装执行顺序](installer-flow.md) | 要理解 bootstrap、precheck、fallback 和退出码 | 不列应用版本和 fallback 文件名 |
| [Skill 导入契约](skill-import.md) | 要理解 Profile、`.skill-meta.json`、三态去重和 SQLite 注册 | 不维护 Skill 清单 |
