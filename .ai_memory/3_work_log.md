...
- TUI 运行模式首页在“现代化控制台安装向导”下补充工具说明和仓库地址；该说明只显示在首屏，不进入工作台和后续复选页。
- 远程 `vibe-coding-setup.cmd` 入口补充 `-RefreshBootstrapDependencies`，避免 `%TEMP%` 里旧版 `modules/common.psm1` / `manifest/apps.json` 让最新 UI 或安装逻辑不生效。
