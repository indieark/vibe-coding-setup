...
- 去掉默认安装里重复的 `[当前/总数]` 阶段提示：工作区准备、配置导入、插件安装只保留区域标题，最终完成提示也改为分区标题样式；主流程大区域之间保留两行空白，区域内小分块保持一行空白；套件输入区提示改为三行，套件选项从名称 / 数量 / 说明三行压缩为名称含说明 + 数量两行，所有套件也只显示 Skill / MCP / CLI 数量。
